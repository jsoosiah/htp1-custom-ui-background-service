...
const settingsActiveTab = ref(
  localStorage.getItem('settingsActiveTab')
    ? parseInt(localStorage.getItem('settingsActiveTab'))
    : 0
);

// EQ group by setting - 0 to group by channel, 1 to group by band
const eqGroupBy = ref(
  localStorage.getItem('eqGroupBy') ? parseInt(localStorage.getItem('eqGroupBy')) : 0
);

// Show channel mute controls on Calibration tab
const showChannelMuteControls = ref(
  localStorage.getItem('showChannelMuteControls')
    ? localStorage.getItem('showChannelMuteControls')
    : false
);
