...
function findServers(ipList, timeout) {
  console.log('findServers', ipList);
  // var ipCurrent = +ipLow;
  ...
  function tryOne(ip) {
    ++numInFlight;
    var address = 'ws://' + ip + '/ws/controller';
  ...
  function next() {
    ...
    //   tryOne(ipCurrent++);
    // }
    while (ipCurrentIndex < ipList.length && !exitFlag) {
      if (exitFlag) {
        break;
      }
      tryOne(ipList[ipCurrentIndex++]);
    }
...
watch(websocketurl, (newWebsocketurl) => {
  if (ws) {
    ws.close();
  }
  initialize();
