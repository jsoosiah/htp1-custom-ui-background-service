...
const commandKeys = ['cmda', 'cmdb', 'cmdc', 'cmdd', 'preset1', 'preset2', 'preset3', 'preset4'];
...
watch(commandsAwaitingResponse, (newCommandsAwaitingResponse) => {
...
watch(commandsReceived, (newCommandsReceived) => {
...
function applyProductRules() {
  ...
  if (spg) {
    ...
    if (
      !spg.lrs.present &&
      spg.lrtm.present &&
      (spg.lrtf.present || spg.lrtr.present || spg.lrhf.present || spg.lrhr.present)
    ) {
    ...
    for (const inputKey in mso.value.inputs) {
      if (mso.value.inputs) {
...
const visibleDiracSlots = computed(() => {
  const filtered = {};
  if (mso.value.personalize?.diracSlots) {
    for (const slotIndex in mso.value.personalize?.diracSlots) {
...
const visibleMacros = computed(() => {
  const filtered = {};
  if (mso.value.personalize?.macros) {
    for (const key in mso.value.personalize?.macros) {
...
function setDefaultsBeforePowerDown() {
  ...
  if (defaultUpmix && mso.value?.upmix.select !== defaultUpmix) {
    return [{ op: 'replace', path: '/upmix/select', value: defaultUpmix }];
  }
...
function powerOn() {
  commandsToSend.value = [{ op: 'replace', path: '/powerIsOn', value: true }];

  return true;
}

function setVolume(volumeLevel) {
  const volInt = convertInt(
    volumeLevel,
    mso.value.powerOnVol,
    mso.value.cal?.vpl,
    mso.value.cal?.vph
  );
...
function toggleSpeakerGroup(spkCode) {
  return patchMso(
    'replace',
    `/speakers/groups/${spkCode}/present`,
    !mso.value.speakers.groups[spkCode].present
  );
}
...
function setLipsyncDelay(lipsyncDelay) {
  const delay = convertInt(lipsyncDelay, 0, 0, 200);
...
function setUserDelay(channel, delay) {
  return patchMso(
    'replace',
    `/cal/slots/${mso.value.cal.currentdiracslot}/channels/${channel}/delay`,
    parseFloat(delay)
  );
}
...
function setUserTrim(channel, trim) {
  return patchMso(
    'replace',
    `/cal/slots/${mso.value.cal.currentdiracslot}/channels/${channel}/trim`,
    parseFloat(trim)
  );
}
...
function setMuteChannelOn(channel) {
  if (currentDiracSlot.value.channels[channel].mute === undefined) {
    ...
    const preMuteTrim = patchMso(
      'add',
      `/cal/slots/${mso.value.cal.currentdiracslot}/channels/${channel}/preMuteTrim`,
      currentDiracSlot.value.channels[channel].trim
    );
    // set mute flag to true
    const mute = patchMso(
      'add',
      `/cal/slots/${mso.value.cal.currentdiracslot}/channels/${channel}/mute`,
      true
    );
...
function setMuteAllChannelsOn() {
  let result = true;
  for (const channel in currentDiracSlot.value.channels) {
...
function setSineFrequency(freq) {
  const freqValue = convertInt(freq, 440, 10, 20000);
  return patchMso('replace', `/sgen/sinehz`, freqValue);
}

function setSineAmplitude(gain) {
  const gainValue = convertFloat(gain, -20, -140, 0);
...
function setPEQFilterType(channel, slot, filterType) {
  return patchMso(
    'replace',
    `/peq/slots/${slot}/channels/${channel}/FilterType`,
    parseInt(filterType)
  );
}
...
function _setInputDefaultUpmix(input, defaultUpmix, op) {
  if (!op) {
    op = Object.prototype.hasOwnProperty.call(mso.value.inputs[input], 'defaultUpmix')
      ? 'replace'
      : 'add';
  }
...
function _setInputVolumeTrim(input, trim, op) {
  ...
  const trimValue = convertInt(trim, 0, -12, 12);
...
function setInputDelay(input, delayStr) {
  const delay = convertInt(delayStr, 0, 0, 200);
...
function toggleFastStartPassThrough() {
  return patchMso(
    'replace',
    `/fastStartPassThrough`,
    mso.value.fastStartPassThrough === 'off' ? 'on' : 'off'
  );
}
...
function toggleAdvancedInputSettings() {
  return patchMso(
    'replace',
    `/stat/displayAdvancedSettings`,
    !mso.value.stat.displayAdvancedSettings
  );
}
...
function setBassLpf(lpf) {
  const lpfValue = convertInt(lpf, 120, 40, 200);
  bmlfecApplied = false;
...
export default function useMso() {
  return {
    ...
    setUpmix,
    toggleUpmixHomevis,
    toggleUpmixCenterSpread,
    toggleUpmixWideSynth,
    setUpmixWideSynthOff,
    setUpmixWideSynthOn,
    setAuroMaticPreset,
    setAuroMaticStrength,
    setDefaultAuroMaticStrength,
    toggleReinforceBass,
    setReinforceBassOn,
    setReinforceBassOff,
    setNextNightMode,
    ...
    setUserTrim,
    toggleMuteChannel,
    setMuteAllChannelsOff,
    setMuteAllChannelsOn,
    toggleAllMuteChannels,
    toggleSignalGenerator,
    setSignalGeneratorOff,
    setSignalGeneratorOn,
    setSignalGeneratorChannel,
    setSignalGeneratorChannel2,
    setSignalGeneratorSignalType,
    setSineFrequency,
    setSineAmplitude,
    toggleToneControl,
    setBassCornerFrequency,
    setTrebleCornerFrequency,
    setBassBoostCutLevel,
    setTrebleBoostCutLevel,
    setLoudnessCalibration,
    setLoudnessCurve,
    toggleGlobalPEQ,
    setGlobalPEQOff,
    setGlobalPEQOn,
    setPEQSlot,
    setPEQCenterFrequency,
    setPEQGain,
    setPEQQuality,
    setPEQFilterType,
    resetPEQ,
    setInputLabel,
    toggleInputVisible,
    setInputFormatDetectOption,
    toggleInputUHD,
    setInputDefaultUpmix,
    setInputVolumeTrim,
    setInputDelay,
    setBluetoothDiscoverableTime,
    enableBluetoothDiscovery,
    toggleCEC,
    setCECOff,
    setCECOn,
    setTVSoundSrcDefault,
    toggleCECAllowPowerKey,
    toggleCECAllowVolKey,
    toggleCECAllowSysAudioOff,
    toggleCECAllowInputChange,
    toggleCECAllowStandby,
    toggleFastStart,
    setFastStartOff,
    setFastStartOn,
    toggleFastStartPassThrough,
    setFastStartPassThroughOff,
    setFastStartPassThroughOn,
    setUnitName,
    setPowerOnVol,
    setFrontPanelBrightness,
    toggleVideoStatusHomePage,
    toggleExtendedAudioStatus,
    toggleAdvancedInputSettings,
    toggleSupportTools,
    importMsoPatchList,
    saveRecordedCommands,
    saveExtraRecordedCommands,
    createExtraMacro,
    deleteExtraMacro,
    toggleShortcut,
    toggleShowMode,
...
function filterMatchingCommand(cmdList, newCmd) {
  return cmdList.filter((cmd) => {
    return !(cmd.op === newCmd.op && cmd.path === newCmd.path && isEqual(cmd.value, newCmd.value));
  });
}
...
function convert(convertFunction, value, defaultValue, minValue, maxValue) {
  const intValue = convertFunction(value);
